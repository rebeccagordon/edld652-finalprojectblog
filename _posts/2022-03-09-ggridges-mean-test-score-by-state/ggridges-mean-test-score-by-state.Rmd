---
title: "ggridges: mean test scores above or at proficent by state"
description: |
author:
  - name: Rebecca Gordon
date: 2022-03-09
...
- Design choices: 
	- Color-blind friendly gradient palette
	- My original idea was to show differences in test scores by gender (e.g. male and female). However, I decided to show all groups in one plot.
	- Expanded height and width so that the axes are legible.
- Findings: There are no clear regional differences. Some eastern states have highest proficiency (Virginia and Maryland) and some have lowest (Rhode Island and New Hampshire).
- Prior versions: I initially wanted to show the ridge lines by state and thought that the gradient palette was very striking and visually representative of the mean proficient scores.
...
::: {.panel}
## Plot {.panel-name}
```{r rebecca-plots, fig.height=12, fig.width=10, echo=FALSE}
df_pivot %>%
...
```{r rebecca-plots-prior1, echo=FALSE}
df_pivot %>%
...
```{r rebecca-plots-prior2, echo=FALSE}
df_pivot %>%
	ggplot(aes(x = mean_pct,
						 y = fct_reorder(STNAM.y, mean_pct),
						 fill = mean_pct)) +
	geom_density_ridges(scale = 2, size = 0.3, rel_min_height = 0.001) +
	scale_fill_viridis_c(name = "%", option = "C") +
	coord_cartesian(clip = "off") +
	theme_ridges(grid = FALSE) +
	labs(title = 'Mean percentage of students that scored at or above proficient',
...
::: {.panel}
## Code {.panel-name}
```{r rebecca-plots-code, fig.show='hide'}
df_pivot %>%
	ggplot(aes(x = mean_pct,
						 y = fct_reorder(STNAM.y, mean_pct),
						 fill = stat(x))) +
